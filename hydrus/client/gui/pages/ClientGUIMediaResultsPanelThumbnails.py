...
def GetThumbnailPanelBridge( win: QW.QWidget, page_key: bytes, page_manager: ClientGUIPageManager.PageManager, media_results: list[ ClientMediaResult.MediaResult ] ) -> "MediaResultsPanelThumbnails | MediaResultsPanelThumbnailsGraphicsViewTest":
    
    if CG.client_controller.new_options.GetBoolean( 'test_thumbnails_graphics_view' ):
        
        return MediaResultsPanelThumbnailsGraphicsViewTest( win, page_key, page_manager, media_results )
        
    else:
        
        return MediaResultsPanelThumbnails( win, page_key, page_manager, media_results )
...
class MediaResultsPanelThumbnails( ClientGUIMediaResultsPanel.MediaResultsPanel ):
    ...
    def _DrawCanvasPage( self, page_index, canvas_page ):
        ...
        for ( thumbnail_index, thumbnail ) in page_thumbnails:
            ...
            if hash in self._hashes_faded and thumbnails_cache.HasThumbnailCached( thumbnail ):
...
class MediaResultsPanelThumbnailsGraphicsViewTest( ClientGUIMediaResultsPanel.MediaResultsPanelGraphicsViewTest ):
    ...
    def __init__( self, parent, page_key, page_manager: ClientGUIPageManager.PageManager, media_results ):
        ...
        self._media_to_thumbnails: dict[ ClientMedia.Media, ThumbnailGraphicsViewTest ] = {} # associates medias with their Thumbnails (which are Qt GraphicsItems and live in the QGraphicsScene)
    ...
    def _ArrangeThumbnails( self ):
        ...
        thumbnails_sorted = [ self._media_to_thumbnails[ media ] for media in self._sorted_media ]
        
        new_scene_rect = self._thumbnail_layout.ArrangeThumbnails( thumbnails_sorted, self._CollectLayoutParams() )
    ...
    def _CollectLayoutParams( self ) -> ClientGUIThumbnailLayouts.LayoutParams:
        ...
        return ClientGUIThumbnailLayouts.LayoutParams(
            ...
            scroll_direction = ClientGUIThumbnailLayouts.ScrollDirection.VERTICAL,
            content_alignment = QC.Qt.AlignmentFlag.AlignLeft, # layouts will ignore alignments that don't make sense for them or with the selected scroll direction
            zoom = 1.0
        )
    ...
    def AddMediaResults( self, page_key, media_results ):
        ...
        if page_key == self._page_key:
            ...
            if len( thumbnails ) > 0:
                ...
                CG.client_controller.thumbnails_cache_graphics_view_test.Waterfall( self._page_key, thumbnails )
...
class ThumbnailGraphicsViewTest( QW.QGraphicsItem ):
    ...
    def _PaintThumbnailContent( self, painter: QG.QPainter, media: ClientMedia.Media, media_panel: ClientGUIMediaResultsPanel.MediaResultsPanel ) -> None:
        ...
        if media.GetDisplayMedia() is None:
            ...
            thumbnail_hydrus_bmp = thumbnails_cache.GetHydrusSpecialThumbnail( content_size )
