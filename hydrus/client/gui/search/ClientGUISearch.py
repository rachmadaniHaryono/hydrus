...
class EditPredicatesPanel( ClientGUIScrolledPanels.EditPanel ):
    ...
    def __init__( self, parent, predicates: collections.abc.Collection[ ClientSearchPredicate.Predicate ], empty_file_search_context: ClientSearchFileSearchContext.FileSearchContext | None = None, for_metadata_conditional: bool = False ):
        ...
        predicates_list.sort( key = lambda p: p.GetMagicSortValue() )
...
class FleshOutPredicatePanel( ClientGUIScrolledPanels.EditPanel ):
    ...
    def __init__( self, parent, predicate: ClientSearchPredicate.Predicate ):
        ...
        if predicate_type == ClientSearchPredicate.PREDICATE_TYPE_SYSTEM_IMPORT_TIME:
            ...
            static_pred_buttons.append( ClientGUIPredicatesSingle.StaticSystemPredicateButton( self, ( ClientSearchPredicate.Predicate( ClientSearchPredicate.PREDICATE_TYPE_SYSTEM_DURATION, ClientNumberTest.NumberTest.STATICCreateFromCharacters( '=', 0 ) ), ), show_remove_button = False ) )
            static_pred_buttons.append( ClientGUIPredicatesSingle.StaticSystemPredicateButton( self, ( ClientSearchPredicate.Predicate( ClientSearchPredicate.PREDICATE_TYPE_SYSTEM_FRAMERATE, ClientNumberTest.NumberTest( operator = ClientNumberTest.NUMBER_TEST_OPERATOR_APPROXIMATE_ABSOLUTE, value = 30, extra_value = 1 ) ), ), show_remove_button = False ) )
            static_pred_buttons.append( ClientGUIPredicatesSingle.StaticSystemPredicateButton( self, ( ClientSearchPredicate.Predicate( ClientSearchPredicate.PREDICATE_TYPE_SYSTEM_FRAMERATE, ClientNumberTest.NumberTest( operator = ClientNumberTest.NUMBER_TEST_OPERATOR_APPROXIMATE_ABSOLUTE, value = 60, extra_value = 1 ) ), ), show_remove_button = False ) )
...
class TagContextButton( ClientGUICommon.BetterButton ):
    ...
    def __init__( self, parent: QW.QWidget, tag_context: ClientSearchTagContext.TagContext, use_short_label = False, combined_tags_ok = True ):
        
        self._tag_context = ClientSearchTagContext.TagContext()
        
        self._use_short_label = use_short_label
        self._combined_tags_ok = combined_tags_ok
    ...
    def _Edit( self ):
        ...
        service_types_in_order = [ HC.LOCAL_TAG, HC.TAG_REPOSITORY ]
        
        if self._combined_tags_ok:
            
            service_types_in_order.append( HC.COMBINED_TAG )
